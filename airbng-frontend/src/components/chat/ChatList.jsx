  ...
  useEffect(() => {
    const onHint = (e) => {
      ...
      setItems((prev) => {
        ...
        const next = {
          ...
          lastMessage: {
            ...last,
            preview: preview ?? last.preview ?? last.text,
            text: preview ?? last.text,
            sentAt: sentAt ?? last.sentAt,
          },
          cachedUnread:
            unreadTotal != null
              ? unreadTotal
              : Math.max(0, (target.cachedUnread || 0) + 1),
        };
        // 최신으로 끌어올리기
        arr.splice(idx, 1);
        arr.unshift(next);
    ...
    const onRead = (e) => {
      ...
      setItems((prev) =>
        Array.isArray(prev)
          ? prev.map((it) =>
              it.convId === convId ? { ...it, cachedUnread: 0 } : it
            )
          : prev
      );
    };
        ...
        <EmptyState
          ...
          action={
            <button
              ...
              onClick={() => navigate("/page/chat/new")}
            >
              대화 시작
            </button>
          }
