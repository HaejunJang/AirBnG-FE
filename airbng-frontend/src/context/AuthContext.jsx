...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (getRememberMe()) {
        try {
          const res = await httpPublic.post("/reissue", null, {
  ...
  const login = useCallback(
    async (credentials, opts = {}) => {
      const remember = credentials?.remember ?? opts?.remember ?? false; // remember 받기
      const res = await httpPublic.post("/login", credentials, {
        withCredentials: true,
      });
      ...
      const nextUser = {
        ...
        profileImageUrl:
          body?.profileImageUrl ?? baseUser.profileImageUrl ?? "",
      };
  ...
  const logout = useCallback(async () => {
    try {
      await httpPublic.post("/logout", null, { withCredentials: true });
    } catch {}
    clearTokens();
    clearUserProfile();
...
export const getCurrentUserIdFromToken = () => {
  ...
  try {
    ...
    // 여러 가능한 필드명 시도
    const userId =
      payload.memberId || payload.id || payload.sub || payload.userId;
