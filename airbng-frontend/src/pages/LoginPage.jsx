...
const cdKeyFor = (email) =>
  `${COOLDOWN_KEY}:${(email || "").toLowerCase().trim()}`;
...
export default function LoginPage() {
  ...
  const { modalState, hideModal, showError, showWarning, showSuccess } =
    useModal();
  ...
  const startCooldown = useCallback(
    (sec) => {
      const end =
        Date.now() + (Number(sec) > 0 ? Number(sec) : DEFAULT_COOLDOWN) * 1000;
  ...
  const getFailCount = () =>
    Number(sessionStorage.getItem(FAIL_COUNT_KEY) || "0");
          ...
          <div className="signup-section">
            <p className="signup-text">
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/page/signup?redirect=${encodeURIComponent(redirect)}`
                  )
                }
              >
                회원가입
