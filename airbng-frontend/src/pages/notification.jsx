    ...
    <div className="notification-item">
      <div className="notification-content">
        <div className="notification-header">
          <span
            className={`notification-type ${classMap[notification.type] || ""}`}
          >
        ...
        <div className="notification-details">
          예약번호: {notification.reservationId} | 사용자:{" "}
      ...
      <div className="clear-all-container">
        <button
          ...
          className={`clear-all-button ${
            visibleNotifications.length === 0 ? "disabled" : ""
          }`}
        >
...
const NotificationApp = () => {
  ...
  const clearAllNotifications = useCallback(() => {
    ...
    setDeletedRef((prevMap) => {
      ...
      alarms.forEach((notification) => {
        ...
        if (
          !existingDeletedTime ||
          now - existingDeletedTime >= TWENTY_THREE_HOURS
        ) {
          newMap.set(key, now);
          newDeletedCount++;
        }
  ...
  useEffect(() => {
    ...
    const handleAlarm = async (event) => {
      ...
      if (
        receiverId === myId &&
        window.location.pathname === "/page/notification"
      ) {
        try {
