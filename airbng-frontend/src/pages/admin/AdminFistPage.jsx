...
const AdminDashboard = () => {
    ...
    const [activeMenu, setActiveMenu] = useState('보관소 심사'); // 기본값을 '보관소 심사'로 변경
    ...
    const handleMenuClick = (item) => {
        ...
        if (item.subItems.length === 0) {
            // 서브메뉴가 없는 경우 바로 페이지 이동
            setActiveSubMenu('');
            navigate(item.path);
        } else {
            // 서브메뉴가 있는 경우 서브메뉴 초기화
            setActiveSubMenu('');
        }
    };
                    ...
                    <div className={styles.textGray}>
                        ...
                        <div className={styles.menuGrid}>
                            {menuItems.find(item => item.name === '매출')?.subItems.map((subItem, index) => (
                                <div key={index} className={styles.menuCard} onClick={() => handleSubMenuClick(subItem)}>
                                    ...
                                    <p className={styles.menuCardDesc}>
                                        ...
                                        {subItem.name === '결제 수단별 매출' && '결제 수단별 매출 현황을 분석하세요'}
